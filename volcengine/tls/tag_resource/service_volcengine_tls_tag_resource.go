	...
	tagCallback := ve.Callback{
		Call: ve.SdkCall{
			...
			BeforeCall: func(d *schema.ResourceData, client *ve.SdkClient, call ve.SdkCall) (bool, error) {
				// Convert single resource_id to ResourcesList array as required by API
				resourceId := d.Get("resource_id").(string)
				(*call.SdkParam)["ResourceType"] = d.Get("resource_type")
				(*call.SdkParam)["ResourcesList"] = []string{resourceId}
			...
			AfterCall: func(d *schema.ResourceData, client *ve.SdkClient, resp *map[string]interface{}, call ve.SdkCall) error {
				// AddTagsToResource doesn't return an ID, use resource ID as tag resource ID
				resourceId := d.Get("resource_id").(string)
				d.SetId(resourceId)
				return nil
			},
		...
		Call: ve.SdkCall{
			...
			BeforeCall: func(d *schema.ResourceData, client *ve.SdkClient, call ve.SdkCall) (bool, error) {
				...
				if len(tagKeyList) > 0 {
					...
					(*call.SdkParam)["ResourcesList"] = resourcesList
					(*call.SdkParam)["TagKeyList"] = tagKeyList
			...
			AfterCall: func(d *schema.ResourceData, client *ve.SdkClient, resp *map[string]interface{}, call ve.SdkCall) error {
				...
				return resource.Retry(5*time.Minute, func() *resource.RetryError {
					...
					if tags, ok := tagsMap["ResourceTags"].([]map[string]interface{}); ok {
						removedTagKeys := (*call.SdkParam)["TagKeyList"].([]string)
