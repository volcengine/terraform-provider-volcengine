package certificate_test

import (
	"github.com/hashicorp/terraform-plugin-sdk/helper/resource"
	"github.com/volcengine/terraform-provider-volcengine/volcengine"
	"github.com/volcengine/terraform-provider-volcengine/volcengine/clb/certificate"
	"testing"
)

const testAccVolcengineCertificatesDatasourceConfig = `
resource "volcengine_certificate" "foo" {
    certificate_name = "acc-test-certificate-${count.index}"
  	description = "acc-test-demo"
  	public_key = "-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----"
	private_key = "-----BEGIN RSA PRIVATE KEY-----\\n-----END RSA PRIVATE KEY-----"
  	project_name = "default"
  	tags  {
    	key = "k1"
    	value = "v1"
  	}
	count = 3
}

data "volcengine_certificates" "foo"{
    ids = volcengine_certificate.foo[*].id
}
`

func TestAccVolcengineCertificatesDatasource_Basic(t *testing.T) {
	resourceName := "data.volcengine_certificates.foo"

	acc := &volcengine.AccTestResource{
		ResourceId: resourceName,
		Svc:        &certificate.VolcengineCertificateService{},
	}

	resource.ParallelTest(t, resource.TestCase{
		PreCheck: func() {
			volcengine.AccTestPreCheck(t)
		},
		Providers: volcengine.GetTestAccProviders(),
		Steps: []resource.TestStep{
			{
				Config: testAccVolcengineCertificatesDatasourceConfig,
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(acc.ResourceId, "certificates.#", "3"),
				),
			},
		},
	})
}
