...
func (s *Service) readAttachments(m map[string]interface{}) (data []interface{}, err error) {
	var (
...
func (s *Service) ReadResources(m map[string]interface{}) (data []interface{}, err error) {
	...
	return ve.WithPageNumberQuery(m, "PageSize", "PageNumber", 20, 1, func(condition map[string]interface{}) ([]interface{}, error) {
		...
		for index, element := range data {
			attach, err := s.readAttachments(map[string]interface{}{
				"TransitRouterId": element.(map[string]interface{})["TransitRouterId"],
			})
			if err != nil {
				return nil, err
			}
			data[index].(map[string]interface{})["TransitRouterAttachments"] = attach
		}
		return data, err
	})
}
	...
	return &resource.StateChangeConf{
		...
		Refresh: func() (result interface{}, state string, err error) {
			...
			for _, v := range failStates {
				if v == status.(string) {
					return nil, "", fmt.Errorf("TransitRouter status  error, status:%s", status.(string))
				}
			}
	...
	callback := ve.Callback{
		Call: ve.SdkCall{
			...
			AfterCall: func(d *schema.ResourceData, client *ve.SdkClient, resp *map[string]interface{}, call ve.SdkCall) error {
				id, _ := ve.ObtainSdkValue("Result.TransitRouterId", *resp)
				d.SetId(id.(string))
				return nil
			},
	...
	callback := ve.Callback{
		Call: ve.SdkCall{
			...
			SdkParam: &map[string]interface{}{
				"TransitRouterId": resourceData.Id(),
			},
			ExecuteCall: func(d *schema.ResourceData, client *ve.SdkClient, call ve.SdkCall) (*map[string]interface{}, error) {
				logger.Debug(logger.RespFormat, call.Action, call.SdkParam)
				return s.Client.UniversalClient.DoCall(getUniversalInfo(call.Action), call.SdkParam)
				...
				return resource.Retry(15*time.Minute, func() *resource.RetryError {
					...
					if callErr != nil {
						if ve.ResourceNotFoundError(callErr) {
							...
							return resource.NonRetryableError(fmt.Errorf("error on  reading transit router on delete %q, %w", d.Id(), callErr))
						}
					}
