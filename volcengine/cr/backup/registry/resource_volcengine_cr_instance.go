...
func ResourceVolcengineCrRegistry() *schema.Resource {
	resource := &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			"name": {
				...
				Required:    true,
				Description: "The name of CrRegistry.",
			},
			"delete_immediately": {
				...
				Description: "Whether delete instance immediately.",
			},
			"password": {
				Type:        schema.TypeString,
				Required:    true,
	...
	dataSource := DataSourceVolcengineCrRegistrys().Schema["instances"].Elem.(*schema.Resource).Schema
