...
func (s *VolcengineVeecpClusterService) ReadResources(m map[string]interface{}) (data []interface{}, err error) {
	var (
...
func (s *VolcengineVeecpClusterService) ReadResource(resourceData *schema.ResourceData, id string) (data map[string]interface{}, err error) {
	var (
		results []interface{}
		ok      bool
	)
	...
	if len(data) == 0 {
		return data, fmt.Errorf("veecp_cluster %s not exist ", id)
	}
	return data, err
}
...
func (VolcengineVeecpClusterService) WithResourceResponseHandlers(d map[string]interface{}) []ve.ResourceResponseHandler {
