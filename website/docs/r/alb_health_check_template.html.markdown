...
* `health_check_port` - (Optional) The port for health check. 0 means use backend server port for health check, 1-65535 means use the specified port.
* `health_check_protocol` - (Optional) The protocol of health check, support HTTP and TCP.
...
* `project_name` - (Optional) The project name to which the health check template belongs.
* `tags` - (Optional) Tags.
