...
```
$ terraform import vestack_vpn_connection.default vgc-3tex2x1cwd4c6c0v****
```

*/

func ResourceVestackVpnConnection() *schema.Resource {
	resource := &schema.Resource{
		Create: resourceVestackVpnConnectionCreate,
		Read:   resourceVestackVpnConnectionRead,
		Update: resourceVestackVpnConnectionUpdate,
		Delete: resourceVestackVpnConnectionDelete,
	...
	dataSource := DataSourceVestackVpnConnections().Schema["vpn_connections"].Elem.(*schema.Resource).Schema
	delete(dataSource, "id")
	ve.MergeDateSourceToResource(dataSource, &resource.Schema)
	return resource
}

func resourceVestackVpnConnectionCreate(d *schema.ResourceData, meta interface{}) (err error) {
	connectionService := NewVpnConnectionService(meta.(*ve.SdkClient))
	err = connectionService.Dispatcher.Create(connectionService, d, ResourceVestackVpnConnection())
	if err != nil {
		return fmt.Errorf("error on creating Vpn Connections %q, %s", d.Id(), err)
	}
	return resourceVestackVpnConnectionRead(d, meta)
}

func resourceVestackVpnConnectionRead(d *schema.ResourceData, meta interface{}) (err error) {
	connectionService := NewVpnConnectionService(meta.(*ve.SdkClient))
	err = connectionService.Dispatcher.Read(connectionService, d, ResourceVestackVpnConnection())
	if err != nil {
...
func resourceVestackVpnConnectionUpdate(d *schema.ResourceData, meta interface{}) (err error) {
	connectionService := NewVpnConnectionService(meta.(*ve.SdkClient))
	err = connectionService.Dispatcher.Update(connectionService, d, ResourceVestackVpnConnection())
	if err != nil {
...
func resourceVestackVpnConnectionDelete(d *schema.ResourceData, meta interface{}) (err error) {
	connectionService := NewVpnConnectionService(meta.(*ve.SdkClient))
	err = connectionService.Dispatcher.Delete(connectionService, d, ResourceVestackVpnConnection())
	if err != nil {
