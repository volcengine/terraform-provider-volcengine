...
```
$ terraform import vestack_scaling_configuration.default scc-ybkuck3mx8cm9tm5yglz
```

*/

func ResourceVestackScalingConfiguration() *schema.Resource {
	resource := &schema.Resource{
		Create: resourceVestackScalingConfigurationCreate,
		Read:   resourceVestackScalingConfigurationRead,
		Update: resourceVestackScalingConfigurationUpdate,
		Delete: resourceVestackScalingConfigurationDelete,
	...
	dataSource := DataSourceVestackScalingConfigurations().Schema["scaling_configurations"].Elem.(*schema.Resource).Schema
	delete(dataSource, "id")
	ve.MergeDateSourceToResource(dataSource, &resource.Schema)
	return resource
}

func resourceVestackScalingConfigurationCreate(d *schema.ResourceData, meta interface{}) (err error) {
	scalingConfigurationService := NewScalingConfigurationService(meta.(*ve.SdkClient))
	err = scalingConfigurationService.Dispatcher.Create(scalingConfigurationService, d, ResourceVestackScalingConfiguration())
	if err != nil {
		return fmt.Errorf("error on creating ScalingConfiguration %q, %s", d.Id(), err)
	}
	return resourceVestackScalingConfigurationRead(d, meta)
}

func resourceVestackScalingConfigurationRead(d *schema.ResourceData, meta interface{}) (err error) {
	scalingConfigurationService := NewScalingConfigurationService(meta.(*ve.SdkClient))
	err = scalingConfigurationService.Dispatcher.Read(scalingConfigurationService, d, ResourceVestackScalingConfiguration())
	if err != nil {
...
func resourceVestackScalingConfigurationUpdate(d *schema.ResourceData, meta interface{}) (err error) {
	scalingConfigurationService := NewScalingConfigurationService(meta.(*ve.SdkClient))
	err = scalingConfigurationService.Dispatcher.Update(scalingConfigurationService, d, ResourceVestackScalingConfiguration())
	if err != nil {
...
func resourceVestackScalingConfigurationDelete(d *schema.ResourceData, meta interface{}) (err error) {
	scalingConfigurationService := NewScalingConfigurationService(meta.(*ve.SdkClient))
	err = scalingConfigurationService.Dispatcher.Delete(scalingConfigurationService, d, ResourceVestackScalingConfiguration())
	if err != nil {
