	...
	resource := &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			"active": {
				...
				Computed:    true,
				Optional:    true,
				Description: "The active flag of the scaling policy. when the scaling policy be active, the scaling group will also be active.",
			},
			"status": {
			...
			"alarm_policy_condition_metric_unit": {
				Type:         schema.TypeString,
				Optional:     true,
				ValidateFunc: validation.StringInSlice([]string{"Count", "Percent"}, false),
